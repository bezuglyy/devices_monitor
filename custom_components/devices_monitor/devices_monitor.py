...
async def async_setup_runtime(hass: HomeAssistant, entry_id: str, cfg: dict):
    ...
    last_alert_state = {}  # for threshold sensors
    ...
    async def _report(_now):
        lines = []
        if mode in ["switches","both"] and switches:
            lines.extend(await _switch_report_lines())
        if mode in ["sensors","both"]:
            lines.extend(await _sensors_report_lines())

        if lines:
            text = "\n".join(lines)
            # send to sensors alerts channels + switches report channel
            channels = []
            if mode in ["sensors","both"]:
                channels.extend(sn_notify_alerts if sn_notify_alerts else [])
            if mode in ["switches","both"] and sw_notify_report:
                channels.append(sw_notify_report)
            if channels:
                await _broadcast(channels, "📈 Статус устройств", text)
            if sn_notify_tts:
    ...
    for hh, mm in _parse_schedule(cfg.get(CONF_REPORT_SCHEDULE, [])):
    ...
    async def handle_test_notify(call):
        title = call.data.get("title", "Devices Monitor Test")
        message = call.data.get("message", "Это тестовое уведомление от Devices Monitor.")
        # send to all configured channels
        channels = set()
        if sw_notify_report: channels.add(sw_notify_report)
        if sn_notify_alerts: 
            for c in sn_notify_alerts: channels.add(c)
        for c in channels:
            await _notify(c, title, message)
        if sn_notify_tts:
            await _speak(sn_notify_tts, message)
    hass.services.async_register(DOMAIN, "test_notify", handle_test_notify)
